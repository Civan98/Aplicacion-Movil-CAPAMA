...
class ModEmployee extends StatefulWidget {
  @override
  _ModEmployeeState createState() => _ModEmployeeState();
}

class _ModEmployeeState extends State<ModEmployee> {
  ...
  TextEditingController _numEmpController = TextEditingController();
...
void initState()
{
  Empleado employee = Provider.of<UsuarioProvider>(context, listen: false).empleado;
  _nameController.text = employee.nombre;
  _lastnameController.text = employee.apellidos;
  _numEmpController.text = employee.numEmpleado;
  _coloniaController.text = employee.colonia;
  _calleController.text = employee.calle;
  _telefonoController.text = employee.telefono;
  _emailController.text = employee.email;
  _cpController.text = employee.cp;
   _pass = employee.contrasena;
   _correoActual = employee.email;
   _idU = employee.id;
 ...
 Widget _numEmpl(BuildContext context) {
    return Container(
      child: Column(
      ...
      children: [
        ...
        RaisedButton(
          ...
          onPressed: _disablebutton?null: () async{
            Empleado usuariotemp = new Empleado();
           ...
           usuariotemp.email    =  _emailController.text;
            usuariotemp.numEmpleado  = _numEmpController.text;
           usuariotemp.contrasena = _pass;
           usuariotemp.id = _idU;

                //Si esta vacio manda la misma contra
                if(await conn.queryEmailEmployeeDistinct(_correoActual, _emailController.text))
                {
                  if( await conn.updateEmployeeD(usuariotemp))
                  {
                   modSuccess(context);
                  }
                  else
